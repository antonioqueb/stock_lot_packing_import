...
class PackingListImportWizard(models.TransientModel):
    ...
    def _sync_quantities_to_po_lines(self):
        """
        Después de procesar el PL:
        1. Congela x_qty_solicitada_original en la línea de OC (solo la primera vez).
        2. Actualiza product_qty y x_qty_embarcada con la suma real de qty_done
           de las move_lines por producto en este picking.
        """
        ...
        for po_line in po.order_line:
            ...
            # Congelar original SOLO si aún no se ha guardado (primera vez)
            if not po_line.x_qty_solicitada_original:
                vals['x_qty_solicitada_original'] = po_line.product_qty

            # Actualizar la cantidad principal (la que Odoo usa para facturar/recepcionar)
