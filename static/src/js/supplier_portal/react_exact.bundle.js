...
const TR = {
  ...
  zh: {
    ...
    'PI / Pedido': '形式发票 / 订单', '— automático —': '— 自动 —', 'Las PI de esta carga las asigna SOM GROUP por cada pedido.': '本批次的形式发票由SOM GROUP按每个订单分配。', 'Editar estructura': '编辑结构', 'Agregar fila': '添加行', 'Quitar fila': '删除行', 'Falta la foto de bloque: ': '缺少荒料照片: ', '— selecciona naviera —': '— 选择船公司 —', '— selecciona forwarder —': '— 选择货代 —', 'Forwarder': '货代', 'Agente de carga que gestiona este embarque. Solo forwarders del catálogo SOM GROUP.': '管理本货运的货运代理。仅限 SOM GROUP 目录。', 'Compañía que opera el transporte. Solo navieras del catálogo SOM GROUP.': '运营运输的公司。仅限 SOM GROUP 目录船公司。', 'Contenedores': '集装箱', 'Contenedor': '集装箱', 'Cada caja física que viaja en el embarque. Los números son los que están pintados en el contenedor (4 letras + 7 dígitos).': '运输中的每个物理箱体。编号为集装箱上喷涂的(4 字母 + 7 数字)。', 'Captura los números de contenedor en cuanto te los entregue tu agente. Los necesitas antes del packing list.': '代理交付后请立即录入集装箱号。装箱单之前需要。', 'Agregar primer contenedor': '添加首个集装箱', 'Agregar contenedor': '添加集装箱', 'No. Contenedor': '集装箱号', 'No. de Sello': '封条号', 'Sello de seguridad que se rompe al abrir el contenedor.': '打开集装箱时会破坏的安全封条。', 'Peso bruto (kg)': '毛重 (kg)', 'Volumen (m³)': '体积 (m³)', 'No. de paquetes / bultos': '包装件数', 'Dimensión': '尺寸', 'Contenedor sin número': '无编号集装箱',
...
const TabLogistics = ({ ship, updateShip }) => (React.createElement("div", null,
    React.createElement("div", { className: "card" },
        ...
        React.createElement("div", { className: "fld-row cols-3" },
            ...
            React.createElement(Field, { label: "Naviera / Aerol\u00EDnea", required: true, help: "Compa\u00F1\u00EDa que opera el transporte. Solo navieras del cat\u00E1logo SOM GROUP.", helpExample: "COSCO, MSC, Hapag-Lloyd\u2026" },
                ((PORTAL_RAW_PAYLOAD.navieras || []).length
                    ? React.createElement("select", { className: "o_input", value: ship.naviera_id || '', onChange: (e) => {
                            const nid = e.target.value ? parseInt(e.target.value, 10) : false;
                            const nav = (PORTAL_RAW_PAYLOAD.navieras || []).find(n => n.id === nid);
                            updateShip({ naviera_id: nid, shipping_line: nav ? nav.name : '' });
                        } },
                        React.createElement("option", { value: "" }, "\u2014 selecciona naviera \u2014"),
                        (PORTAL_RAW_PAYLOAD.navieras || []).map(n => React.createElement("option", { key: n.id, value: n.id }, n.name)))
            ...
            (PORTAL_RAW_PAYLOAD.forwarders || []).length ? React.createElement(Field, { label: "Forwarder", required: true, help: "Agente de carga que gestiona este embarque. Solo forwarders del cat\u00E1logo SOM GROUP." },
                React.createElement("select", { className: "o_input", value: ship.forwarder_id || '', onChange: (e) => updateShip({ forwarder_id: e.target.value ? parseInt(e.target.value, 10) : false }) },
                    React.createElement("option", { value: "" }, "\u2014 selecciona forwarder \u2014"),
                    (PORTAL_RAW_PAYLOAD.forwarders || []).map(f => React.createElement("option", { key: f.id, value: f.id }, f.name)))) : null,
