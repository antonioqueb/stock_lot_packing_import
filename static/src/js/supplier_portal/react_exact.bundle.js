...
const Step4Sheet = ({ proforma, draft, setDraft, rows, setRows, ship, pendingImages }) => {
    ...
    const rowMatchesFilter = (r) => rowFilter === 'all'
        ? true
        : (rowFilter === 'errors' ? rowHasError(r) : PL_STATE(r).tone !== 'done');
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 18, marginBottom: 14, flexWrap: 'wrap' } },
            ...
            React.createElement("div", { style: { display: 'flex', gap: 6 } }, [
                ...
                style: { display: 'flex', alignItems: 'center', gap: 2, padding: '6px 12px', borderRadius: 9, cursor: 'pointer', fontSize: 12.5, fontWeight: rowFilter === f.key ? 700 : 500, border: rowFilter === f.key ? '1.5px solid var(--accent)' : '1px solid var(--border)', background: rowFilter === f.key ? 'var(--accent-soft)' : 'var(--surface)', color: (f.key === 'errors' && f.count > 0) ? 'var(--danger)' : 'inherit' },
            },
                f.label,
                f.count,
                ")"))),
