    ...
    (draft.blocks || []).forEach((b, bIdx) => {
        ...
        if (mode === 'placa') {
            ...
            if (loose) {
                ...
                const base = { id, product_id: b.product || product.id, tipo, block: lotName, atado: '', plate: '', ref: product.ref || '', thickness: 0, h: 0, w: 0, quantity: +b.count || 0, weight: 0, notes: '', grupo, pedimento: '', container: defaultContainer, container_id: false, pi_header_id: false, photo: false, errors: [], blockStart: true };
                const prev = takePrev(id, base.product_id, base.block, base.grupo);
                if (prev) PL_ROW_KEEP.forEach(k => { if (prev[k] !== undefined) base[k] = prev[k]; });
                base.block = lotName;
                base.grupo = grupo;
                ...
                for (let i = 0; i < n; i++) {
                    const id = `r-${b.id}-e${i}`;
                    const base = { id, product_id: b.product || product.id, tipo, block: lotName, atado: '', plate: '', ref: product.ref || '', thickness: 0, h: 0, w: 0, quantity: 0, weight: 0, notes: '', grupo, pedimento: '', container: defaultContainer, container_id: false, pi_header_id: false, photo: false, errors: [], blockStart: i === 0 };
                    const prev = takePrev(id, base.product_id, base.block, base.grupo);
                    if (prev) PL_ROW_KEEP.forEach(k => { if (prev[k] !== undefined) base[k] = prev[k]; });
                    base.block = lotName;
                    base.grupo = grupo;
