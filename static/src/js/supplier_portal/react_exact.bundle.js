...
const Documents = ({ proforma, setProforma, setRoute }) => {
    ...
    const uploadOne = async (docType, file) => {
        ...
        if (!generalDocAllowed(file)) {
            window.alert('"' + file.name + '": ' + tr('solo se permiten archivos PDF, JPG o PNG.'));
            return false;
        }
        if (file.size > 10 * 1024 * 1024) {
            window.alert('"' + file.name + '" ' + tr('supera el máximo de 10 MB.'));
            return false;
        }
        const { data } = await fileToBase64(file);
        const res = await portalRpc('/supplier/api/v2/upload_document', {
        ...
        if (!res || !res.success) {
            window.alert((res && res.message) || (tr('No se pudo subir') + ' "' + file.name + '".'));
            return false;
        }
    ...
    const uploadMany = async (docType, fileList) => {
        if (!token) {
            window.alert(tr('No se puede subir: el portal no tiene sesión activa.'));
            return;
        }
