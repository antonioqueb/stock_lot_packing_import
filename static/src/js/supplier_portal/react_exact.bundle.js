...
(function () {
    ...
    function normalizePacking(pk, shipment, fallbackProducts) {
        ...
        function rowToUi(r, idx) {
            ...
            return {
                ...
                thickness: s(r.grosor || r.thickness, '').trim(),
                h: n(r.alto || r.h, 0),
                w: n(r.ancho || r.w, 0),
                ...
                pi_header_id: r.pi_header_id || false,
                photo: !!r.has_image,
                errors: []
            };
        }
        ...
        Object.keys(byKey).forEach(function (k) {
            ...
            var pkgKind = (g.indexOf('palet') >= 0 || g.indexOf('pallet') >= 0) ? 'palet' : (g.indexOf('tarima') >= 0 ? 'tarima' : (g.indexOf('caja') >= 0 ? 'caja' : (g.indexOf('suelto') >= 0 ? 'suelto' : '')));
            var mq = g.match(/x\s*(\d+)/);
            var grupoQty = mq ? parseInt(mq[1], 10) : 0;
            if (bb._isPlaca) {
                bb.count = bb._rows;
                bb.packaging = { kind: '', qty: '' };
            } else if (pkgKind === 'caja' || pkgKind === 'palet' || pkgKind === 'tarima') {
                bb.count = 0;
    ...
    function normalize(payload) {
        ...
        var shipments = arr(p.shipments).map(function (sh, idx) {
            ...
            return {
                ...
                naviera_id: sh.naviera_id || false,
                forwarder_id: sh.forwarder_id || false,
                pol_id: sh.pol_id || false,
                pod_id: sh.pod_id || false,
                vessel: s(sh.vessel_name || sh.vessel, ''),
                etd: s(sh.etd, ''),
                eta: s(sh.eta, ''),
                status: s(sh.status, 'draft'),
                // CANDADO DEL PL (backend): la recepción de este embarque ya
                // fue validada en tránsito (o la proforma ya se completó para
                // el proveedor). Nada del packing list se edita ni se guarda.
                is_locked: !!sh.is_locked,
                lock_reason: s(sh.lock_reason, ''),
                received_pickings: arr(sh.received_picking_names),
                notes: s(sh.notes, ''),
    ...
    // Factura de carga (multi-PO): lista de PIs amparadas por este enlace.
    // El proveedor asigna cada fila del PL a una PI (misma mecánica que el
    ...
    window.PORTAL_PI_LABEL = function (fallback) {
        ...
        if (ps.length > 1) {
            var names = ps.map(function (q) { return q.number || q.po_name || ''; }).filter(Boolean);
...
const TR = {
  en: {
    /* i18n-pl-lock */ 'Packing list cerrado': 'Packing list locked', 'Este embarque ya fue recibido en tránsito': 'This shipment has already been received in transit', 'Esta proforma ya fue marcada como completa.': 'This proforma has already been marked as complete.', 'El packing list ya no se puede modificar. Si necesitas corregirlo, pídeselo a tu contacto en SOM GROUP.': 'The packing list can no longer be modified. If you need a correction, ask your SOM GROUP contact.', 'Recibido': 'Received', 'Ocultar detalle': 'Hide detail',
    /* i18n-pl-v2 */ 'rellenar valor': 'fill in value', 'Empaque': 'Packaging', 'm² (por capturar)': 'm² (to capture)', 'Cantidad': 'Quantity', 'No. Palet': 'Pallet No.', 'No. Tarima': 'Skid No.', 'No. Caja': 'Box No.', 'No. Empaque': 'Package No.', 'No. Paquete': 'Package No.', 'No. Pieza': 'Piece No.', 'Área m²': 'Area m²', 'Tono/Lote': 'Shade/Lot', 'Lote': 'Lot', 'Placa #': 'Slab #', 'Largo': 'Length', 'Ancho': 'Width', 'Estado': 'Status', 'PALET': 'PALLET', 'CAJA': 'BOX', 'SUELTO': 'LOOSE', 'TARIMA': 'SKID', 'Pendientes de detalle': 'Pending detail', 'Completo': 'Complete', 'Pendiente de detalle': 'Pending detail', 'm² pendiente': 'm² pending', 'cantidad pendiente': 'quantity pending', 'Falta empaque': 'Packaging missing', 'Listo': 'Ready', 'Incompleto': 'Incomplete', 'Foto opcional': 'Photo optional', 'Sin foto': 'No photo', 'Pendiente m²': 'm² pending', '¿Cómo viene empacado?': 'How is it packaged?', '— elige —': '— choose —', '(vacías)': '(blank)', 'Proforma (PI)': 'Proforma (PI)', 'Por proforma': 'By proforma', 'Suelto': 'Loose', 'Caja': 'Box', 'Palet': 'Pallet', 'Tarima': 'Skid', 'Cantidad total de metros cuadrados': 'Total square meters', 'Cantidad total': 'Total quantity', 'Dividir por tono/lote': 'Split by shade/lot', ' Los metros cuadrados se capturan después.': ' Square meters are captured later.', 'Placa': 'Slab', 'Formato': 'Tile', 'Pieza': 'Piece', 'Llenar detalle pendiente': 'Fill pending detail', 'Llena el detalle pendiente': 'Fill the pending detail', 'Crear y terminar': 'Create and finish', 'Llenar detalle (': 'Fill detail (', ' fila)': ' row)', ' filas)': ' rows)', 'Crear ': 'Create ', ' filas y terminar': ' rows and finish', 'Cada placa y cada empaque (palet/caja) es una fila editable.': 'Each slab and each package (pallet/box) is an editable row.', ' filas completas': ' rows complete', 'No hay filas que detallar todavía.': 'No rows to detail yet.', 'Ver detalle': 'View detail', 'Ocultar': 'Hide', 'Aún no se han generado filas para este packing.': 'No rows generated for this packing yet.', 'Subir/Reemplazar foto': 'Upload/Replace photo', 'Subir/Reemplazar foto de la placa': 'Upload/Replace slab photo', 'Plataforma': 'Platform', '— sin asignar —': '— unassigned —', 'plataforma / camión': 'platform / truck', 'Agregar Bloque': 'Add Block', 'Organiza el contenido del embarque': 'Organize the shipment content', 'El tipo lo define la categoría del producto. En placas, arma los bloques y sube una foto de cada uno. En formatos y piezas, primero indica cómo viene empacado (suelto, caja o palet) y luego la cantidad.': 'The type is set by the product category. For slabs, build the blocks and upload a photo of each. For tiles and pieces, first state how it is packaged (loose, box or pallet) and then the quantity.', 'Captura el detalle de cada fila: una placa o un empaque (palet/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades, y agregas contenedor, fotos y notas.': 'Capture the detail of each row: one slab or one package (pallet/box) per line. Here you adjust dimensions, m² and quantities, and add container, photos and notes.', /* i18n-declaracion */ 'Cómo se envía': 'How it ships', 'Completar detalles': 'Complete details', 'Para empezar, ¿qué productos vas a enviar?': 'To start, which products are you shipping?', 'Declara cómo viene organizado el envío': 'Declare how the shipment is organized', 'Revisa la declaración del envío': 'Review the shipment declaration', 'Completa los detalles del envío': 'Complete the shipment details', 'Indica cómo estás enviando cada producto. Para placas, registra los bloques de origen y sube una foto de cada bloque. Para formatos y piezas, indica el tipo de empaque, la cantidad enviada y, cuando aplique, sube una foto como evidencia del envío.': 'State how you are shipping each product. For slabs, register the source blocks and upload a photo of each block. For tiles and pieces, state the packaging type, the quantity shipped and, when applicable, upload a photo as evidence of the shipment.', 'Confirma lo que declaras enviar en este packing list. Si algo no cuadra, regresa al paso anterior.': 'Confirm what you declare to ship in this packing list. If something is off, go back to the previous step.', 'Detalla lo que envías: una placa o un empaque (palet/tarima/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades enviadas, y agregas contenedor, fotos y notas.': 'Detail what you ship: one slab or one package (pallet/skid/box) per line. Here you adjust dimensions, m² and shipped quantities, and add container, photos and notes.', 'Revisar declaración del envío': 'Review shipment declaration', 'Ajustar declaración': 'Adjust declaration', 'Completar detalles (': 'Complete details (', 'Enviar declaración al comprador': 'Send declaration to buyer', 'Guardar declaración y volver al embarque': 'Save declaration and return to shipment', 'Declarado': 'Declared', 'Declaración incompleta': 'Incomplete declaration', 'Falta foto requerida': 'Required photo missing', 'Falta cantidad enviada': 'Shipped quantity missing', 'Falta indicar empaque': 'Packaging not stated', 'Falta agregar bloque': 'Block missing', 'Falta declarar placas': 'Slabs not declared', 'Falta foto del bloque': 'Block photo missing', 'Cantidad a enviar: ': 'Quantity to ship: ', ' · Declarado: ': ' · Declared: ', '¿Cómo se envía este producto?': 'How is this product shipped?', 'Tipo de empaque': 'Packaging type', 'Metros cuadrados enviados (total)': 'Square meters shipped (total)', 'Cantidad total enviada': 'Total quantity shipped', 'Placas enviadas': 'Slabs shipped', 'Agregar empaque': 'Add package', 'Evidencia opcional del envío': 'Optional shipment evidence', 'Sin evidencia requerida': 'No evidence required', 'Foto del bloque': 'Block photo', 'Evidencia (opc.)': 'Evidence (opt.)', 'Filas de la declaración': 'Declaration rows', 'Con detalles pendientes': 'With pending details', 'Resumen de la declaración': 'Declaration summary', 'Puedes continuar y subirlas después, pero la declaración del envío no se considerará completa hasta que cada bloque tenga al menos una foto.': 'You can continue and upload them later, but the shipment declaration will not be considered complete until every block has at least one photo.', 'Cantidad excedente': 'Extra quantity', 'Declaraste ': 'You declared ', ' de ': ' of ', ' placas. Faltan ': ' slabs. Missing ', ' placas por declarar.': ' slabs to declare.', ' placas. Hay ': ' slabs. There are ', ' placas excedentes. Confirma si deseas continuar con esta diferencia.': ' extra slabs. Confirm if you want to continue with this difference.', ' unidades. Faltan ': ' units. Missing ', ' unidades por declarar.': ' units to declare.', ' unidades. Hay ': ' units. There are ', ' unidades excedentes. Confirma si deseas continuar con esta diferencia.': ' extra units. Confirm if you want to continue with this difference.', 'Confirmo enviar la cantidad excedente': 'I confirm shipping the extra quantity', '2. Cómo se envía': '2. How it ships', '4. Completar detalles': '4. Complete details', ' Los m² se capturan después.': ' m² are captured later.', 'Indica cómo envías cada producto: en placas, bloques de origen con su foto; en formatos y piezas, tipo de empaque y cantidad enviada.': 'State how you ship each product: for slabs, source blocks with their photo; for tiles and pieces, packaging type and quantity shipped.', 'Placas': 'Slabs', 'Formatos': 'Tiles', 'Piezas': 'Pieces', 'Este tipo se organiza por bloques. Registra cada bloque de origen y cuántas placas envías de cada uno.': 'This type is organized by blocks. Register each source block and how many slabs you ship from each.', 'Se declara por empaque. Indica el tipo de empaque y la cantidad enviada; divide por tono o lote si aplica.': 'Declared by package. State the packaging type and the quantity shipped; split by shade or lot if applicable.', 'Se declara por empaque. Indica el tipo de empaque y cuántos empaques envías.': 'Declared by package. State the packaging type and how many packages you ship.',
    /* i18n-fill2 */ 'Todo capturado': 'All captured',
    /* i18n-extra2 */ 'Operación finalizada con avisos': 'Operation finished with warnings', '¡Listo!': 'Done!', 'Confirmado': 'Confirmed', 'Proforma completada': 'Proforma completed', 'No se pudo completar': 'Could not complete', 'La proforma se marcó como completa. SOM GROUP recibió la notificación.': 'The proforma was marked complete. SOM GROUP received the notification.', 'solo se permiten archivos PDF, JPG o PNG.': 'only PDF, JPG or PNG files are allowed.', 'supera el máximo de 10 MB.': 'exceeds the 10 MB maximum.', 'No se pudo subir': 'Could not upload', 'Ocurrió un error al subir el documento: ': 'An error occurred while uploading the document: ', '¿Eliminar': 'Delete', 'Pendiente: ': 'Pending: ', ' filas': ' rows', 'Nombre del bloque #': 'Block name #',
    ...
    'PI / Pedido': 'PI / Order', '— automático —': '— automatic —', 'Las PI de esta carga las asigna SOM GROUP por cada pedido.': 'The PIs of this cargo are assigned by SOM GROUP per order.', 'Editar estructura': 'Edit structure', 'Agregar fila': 'Add row', 'Quitar fila': 'Remove row', 'Falta la foto de bloque: ': 'Block photo missing: ', 'Selecciona…': 'Select…', 'Puerto de Carga (POL)': 'Port of Loading (POL)', 'Puerto Destino (POD)': 'Port of Discharge (POD)', 'Agente de carga del embarque. Solo forwarders con tarifa SOM GROUP.': 'Freight forwarder for this shipment. SOM GROUP tariff catalog only.', 'Solo navieras con tarifa para el forwarder elegido.': 'Only carriers with a tariff for the chosen forwarder.', 'Solo puertos con tarifa para el forwarder elegido.': 'Only ports with a tariff for the chosen forwarder.', 'Solo destinos con tarifa para la ruta elegida.': 'Only destinations with a tariff for the chosen route.', '— selecciona naviera —': '— select carrier —', '— selecciona forwarder —': '— select forwarder —', 'Forwarder': 'Forwarder', 'Agente de carga que gestiona este embarque. Solo forwarders del catálogo SOM GROUP.': 'Freight forwarder managing this shipment. SOM GROUP catalog only.', 'Compañía que opera el transporte. Solo navieras del catálogo SOM GROUP.': 'Company operating the transport. SOM GROUP catalog carriers only.', 'Contenedores': 'Containers', 'Contenedor': 'Container', 'Cada caja física que viaja en el embarque. Los números son los que están pintados en el contenedor (4 letras + 7 dígitos).': 'Each physical box traveling in the shipment. Numbers are those painted on the container (4 letters + 7 digits).', 'Captura los números de contenedor en cuanto te los entregue tu agente. Los necesitas antes del packing list.': 'Enter the container numbers as soon as your agent gives them to you. You need them before the packing list.', 'Agregar primer contenedor': 'Add first container', 'Agregar contenedor': 'Add container', 'No. Contenedor': 'Container No.', 'No. de Sello': 'Seal No.', 'Sello de seguridad que se rompe al abrir el contenedor.': 'Security seal that breaks when opening the container.', 'Peso bruto (kg)': 'Gross weight (kg)', 'Volumen (m³)': 'Volume (m³)', 'No. de paquetes / bultos': 'Packages / bundles', 'Dimensión': 'Dimensions', 'Contenedor sin número': 'Container without number',
    ...
    // Onboarding
    '¡Bienvenido al portal!': 'Welcome to the portal!', 'Aquí vas a registrar los datos del embarque para SOM GROUP. Te guiaremos paso a paso. No tienes que terminar de una sola vez.': 'Here you will register the shipment data for SOM GROUP. We will guide you step by step. You do not have to finish in one go.', 'Tu progreso siempre visible': 'Your progress always visible', 'En el lado izquierdo verás el avance de cada sección con marcas visuales: verde = listo, azul = en progreso, gris = pendiente.': 'On the left you will see each section progress with visual marks: green = done, blue = in progress, gray = pending.', 'Datos generales': 'General data', 'Embarque #1': 'Shipment #1', 'Embarque #2': 'Shipment #2', 'Ayuda contextual en cada campo': 'Contextual help on each field', 'Verás un ícono "?" junto a campos que pueden ser confusos. Pásale el cursor para ver una explicación con ejemplo.': 'You will see a "?" icon next to confusing fields. Hover for an explanation with example.', ' define quién paga el transporte y seguro. ': ' defines who pays transport and insurance. ', 'El packing list es asistido': 'The packing list is assisted', 'En lugar de escribir cientos de líneas, te preguntaremos cuántos bloques cargas y cuántas placas tiene cada uno. Generamos las filas por ti.': 'Instead of writing hundreds of lines, we ask how many blocks you load and how many slabs each has. We generate the rows for you.', '1. Productos': '1. Products', '2. Bloques + fotos': '2. Blocks + photos', '3. Revisión': '3. Review', '4. Llenar placas': '4. Fill slabs', '¿Listo para empezar?': 'Ready to start?', 'Comencemos por los datos generales. Si te trabas, busca el panel de "Guía del paso actual" a la derecha — siempre te dirá qué hacer.': 'Let us start with general data. If you get stuck, look for the "Current step guide" panel on the right — it always tells you what to do.', ' de ': ' of ',
    // Guide panel
    'Tu llenado en 4 etapas': 'Your fill in 4 stages', 'Te recomendamos seguir este orden. Si necesitas saltar a otra sección, también puedes.': 'We recommend following this order. You can skip to another section if needed.', 'Una sola vez al inicio. Identificación de la Proforma, puertos e incoterm.': 'One time at the start. Proforma identification, ports and incoterm.', 'Crea uno o varios. Cada uno con logística, B/L, invoices, contenedores y packing.': 'Create one or several. Each with logistics, B/L, invoices, containers and packing.', 'Sube certificados de calidad y otros papeles generales.': 'Upload quality certificates and other general papers.', 'Última verificación y notificación a SOM GROUP.': 'Final verification and notification to SOM GROUP.', 'Esta sección define identidad y ruta. Si no sabes algo, pregunta a tu agente o déjalo vacío y vuelve después.': 'This section defines identity and route. If unsure, ask your agent or leave empty and come back later.', 'Es el ID que tu sistema usa. Suele comenzar con "PI-".': 'The ID your system uses. Usually starts with "PI-".', 'Origen y destino': 'Origin and destination', 'País y puerto de salida + puerto donde llegará.': 'Country and origin port + destination port.', 'Define quién paga qué. Lo acordaste con tu contacto de SOM GROUP.': 'Defines who pays what. Agreed with your SOM GROUP contact.', 'Pagos y notas': 'Payments and notes', 'Términos de pago y observaciones generales.': 'Payment terms and general notes.', 'Un embarque = un viaje. Puedes dividir la PO en varios embarques si la producción sale en fechas distintas.': 'A shipment = a trip. Split the PO into several shipments if production ships on different dates.', 'Agrega un embarque': 'Add a shipment', 'Hazlo en cuanto tengas el buque o vuelo asignado.': 'Do it as soon as you have the vessel or flight assigned.', 'Llena las 5 secciones': 'Fill the 5 sections', 'Logística, B/L, invoices, contenedores y packing list.': 'Logistics, B/L, invoices, containers and packing list.', 'Sube documentos': 'Upload documents', 'Certificado de origen, fitosanitario, etc.': 'Certificate of origin, phytosanitary, etc.', 'Captura por pestañas': 'Capture by tabs', 'Sigue las pestañas de izquierda a derecha. El packing list es lo más detallado — déjalo para el final.': 'Follow tabs left to right. The packing list is the most detailed — leave it for last.', 'Logística + B/L': 'Logistics + B/L', 'Naviera, buque, fechas y el documento B/L.': 'Carrier, vessel, dates and the B/L document.', 'Invoices': 'Invoices', 'Factura(s) comercial(es). Puede ser una global o varias parciales.': 'Commercial invoice(s). Can be global or several partials.', 'Los números físicos pintados en cada contenedor.': 'The physical numbers painted on each container.', 'Packing list': 'Packing list', 'Asistente paso a paso. Captura placa por placa.': 'Step-by-step wizard. Slab by slab capture.', 'CO, fitosanitario, inspección.': 'CO, phytosanitary, inspection.', 'Documentos que aplican a toda la Proforma. Acepta PDF, JPG, PNG hasta 10 MB.': 'Documents that apply to the whole Proforma. Accepts PDF, JPG, PNG up to 10 MB.', 'Proforma firmada': 'Signed Proforma', 'La que enviaste a SOM GROUP con firma.': 'The one you sent SOM GROUP signed.', 'Certificados de calidad': 'Quality certificates', 'Pruebas técnicas: mineralogía, densidad, absorción.': 'Technical tests: mineralogy, density, absorption.', 'Fotos del producto': 'Product photos', 'Catálogo o muestras a granel.': 'Catalog or bulk samples.', 'Verifica todo': 'Verify everything', 'Una vez marcada como completa, SOM GROUP recibe una notificación. Si después necesitas editar, pídeselo a tu contacto.': 'Once marked complete, SOM GROUP receives a notification. To edit later, ask your contact.', 'Datos clave que se enviarán.': 'Key data that will be sent.', 'Checklist por sección': 'Checklist by section', 'Si algo está en azul, vuelve a esa sección.': 'If something is blue, go back to that section.', 'Marcar como completa': 'Mark as complete', 'Solo se habilita cuando todo está en verde.': 'Only enabled when everything is green.', 'mapa de ruta': 'route map', 'ilustración guía': 'guide illustration',
    // Sidebar
    'Todo listo': 'All done', 'En proceso': 'In progress', '% completado': '% complete', 'PI sin número': 'PI without number',
  },
  zh: {
    /* i18n-pl-lock */ 'Packing list cerrado': '装箱单已锁定', 'Este embarque ya fue recibido en tránsito': '此批货物已在途接收', 'Esta proforma ya fue marcada como completa.': '此形式发票已标记为完成。', 'El packing list ya no se puede modificar. Si necesitas corregirlo, pídeselo a tu contacto en SOM GROUP.': '装箱单已无法修改。如需更正，请联系您在 SOM GROUP 的对接人。', 'Recibido': '已接收', 'Ocultar detalle': '隐藏详情',
    /* i18n-pl-v2 */ 'rellenar valor': '填写数值', 'Empaque': '包装', 'm² (por capturar)': '平方米（待填写）', 'Cantidad': '数量', 'No. Palet': '托盘号', 'No. Tarima': '木托盘号', 'No. Caja': '箱号', 'No. Empaque': '包装号', 'No. Paquete': '包裹号', 'No. Pieza': '件号', 'Área m²': '面积 m²', 'Tono/Lote': '色号/批次', 'Lote': '批次', 'Placa #': '板号', 'Largo': '长', 'Ancho': '宽', 'Estado': '状态', 'PALET': '托盘', 'CAJA': '箱', 'SUELTO': '散装', 'TARIMA': '木托盘', 'Pendientes de detalle': '待填写明细', 'Completo': '完成', 'Pendiente de detalle': '待填写明细', 'm² pendiente': '平方米待填', 'cantidad pendiente': '数量待填', 'Falta empaque': '缺少包装', 'Listo': '就绪', 'Incompleto': '未完成', 'Foto opcional': '照片可选', 'Sin foto': '无照片', 'Pendiente m²': '平方米待填', '¿Cómo viene empacado?': '如何包装？', '— elige —': '— 选择 —', '(vacías)': '(空白)', 'Proforma (PI)': '形式发票 (PI)', 'Por proforma': '按形式发票', 'Suelto': '散装', 'Caja': '箱', 'Palet': '托盘', 'Tarima': '木托盘', 'Cantidad total de metros cuadrados': '总平方米', 'Cantidad total': '总数量', 'Dividir por tono/lote': '按色号/批次拆分', ' Los metros cuadrados se capturan después.': ' 平方米稍后填写。', 'Placa': '板', 'Formato': '砖', 'Pieza': '件', 'Llenar detalle pendiente': '填写待办明细', 'Llena el detalle pendiente': '填写待办明细', 'Crear y terminar': '创建并完成', 'Llenar detalle (': '填写明细（', ' fila)': ' 行）', ' filas)': ' 行）', 'Crear ': '创建 ', ' filas y terminar': ' 行并完成', 'Cada placa y cada empaque (palet/caja) es una fila editable.': '每块板和每个包装（托盘/箱）都是可编辑行。', ' filas completas': ' 行已完成', 'No hay filas que detallar todavía.': '暂无需填写的行。', 'Ver detalle': '查看明细', 'Ocultar': '隐藏', 'Aún no se han generado filas para este packing.': '尚未为此装箱单生成行。', 'Subir/Reemplazar foto': '上传/替换照片', 'Subir/Reemplazar foto de la placa': '上传/替换板照片', 'Plataforma': '平台', '— sin asignar —': '— 未分配 —', 'plataforma / camión': '平台/卡车', 'Agregar Bloque': '添加区块', 'Organiza el contenido del embarque': '组织货物内容', 'El tipo lo define la categoría del producto. En placas, arma los bloques y sube una foto de cada uno. En formatos y piezas, primero indica cómo viene empacado (suelto, caja o palet) y luego la cantidad.': '类型由产品类别决定。板材请配置区块并各上传一张照片。砖和件请先说明包装方式（散装、箱或托盘），再填写数量。', 'Captura el detalle de cada fila: una placa o un empaque (palet/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades, y agregas contenedor, fotos y notas.': '填写每一行的明细：每块板或每个包装（托盘/箱）一行。在此调整尺寸、平方米和数量，并添加货柜、照片和备注。', /* i18n-declaracion */ 'Cómo se envía': '发货方式', 'Completar detalles': '完善明细', 'Para empezar, ¿qué productos vas a enviar?': '首先，您要发运哪些产品？', 'Declara cómo viene organizado el envío': '申报货物的组织方式', 'Revisa la declaración del envío': '检查发货申报', 'Completa los detalles del envío': '完善发货明细', 'Indica cómo estás enviando cada producto. Para placas, registra los bloques de origen y sube una foto de cada bloque. Para formatos y piezas, indica el tipo de empaque, la cantidad enviada y, cuando aplique, sube una foto como evidencia del envío.': '说明每个产品的发运方式。板材请登记来源荒料并为每个荒料上传一张照片。砖和件请注明包装类型、发运数量，并在适用时上传照片作为发货凭证。', 'Confirma lo que declaras enviar en este packing list. Si algo no cuadra, regresa al paso anterior.': '确认您在此装箱单中申报发运的内容。如有问题，请返回上一步。', 'Detalla lo que envías: una placa o un empaque (palet/tarima/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades enviadas, y agregas contenedor, fotos y notas.': '详细填写您发运的货物：每块板或每个包装（托盘/木托盘/箱）一行。在此调整尺寸、平方米和发运数量，并添加货柜、照片和备注。', 'Revisar declaración del envío': '检查发货申报', 'Ajustar declaración': '调整申报', 'Completar detalles (': '完善明细（', 'Enviar declaración al comprador': '向买方提交申报', 'Guardar declaración y volver al embarque': '保存申报并返回货运', 'Declarado': '已申报', 'Declaración incompleta': '申报不完整', 'Falta foto requerida': '缺少必需照片', 'Falta cantidad enviada': '缺少发运数量', 'Falta indicar empaque': '未注明包装', 'Falta agregar bloque': '缺少荒料', 'Falta declarar placas': '未申报板数', 'Falta foto del bloque': '缺少荒料照片', 'Cantidad a enviar: ': '待发数量：', ' · Declarado: ': ' · 已申报：', '¿Cómo se envía este producto?': '该产品如何发运？', 'Tipo de empaque': '包装类型', 'Metros cuadrados enviados (total)': '发运平方米（总计）', 'Cantidad total enviada': '发运总数量', 'Placas enviadas': '发运板数', 'Agregar empaque': '添加包装', 'Evidencia opcional del envío': '发货凭证（可选）', 'Sin evidencia requerida': '无需凭证', 'Foto del bloque': '荒料照片', 'Evidencia (opc.)': '凭证（可选）', 'Filas de la declaración': '申报行数', 'Con detalles pendientes': '明细待完善', 'Resumen de la declaración': '申报摘要', 'Puedes continuar y subirlas después, pero la declaración del envío no se considerará completa hasta que cada bloque tenga al menos una foto.': '您可以继续并稍后上传，但在每个荒料至少有一张照片之前，发货申报不视为完整。', 'Cantidad excedente': '数量超出', 'Declaraste ': '您申报了 ', ' de ': ' / ', ' placas. Faltan ': ' 块板。还差 ', ' placas por declarar.': ' 块板未申报。', ' placas. Hay ': ' 块板。超出 ', ' placas excedentes. Confirma si deseas continuar con esta diferencia.': ' 块板。请确认是否继续保留此差异。', ' unidades. Faltan ': ' 件。还差 ', ' unidades por declarar.': ' 件未申报。', ' unidades. Hay ': ' 件。超出 ', ' unidades excedentes. Confirma si deseas continuar con esta diferencia.': ' 件。请确认是否继续保留此差异。', 'Confirmo enviar la cantidad excedente': '我确认发运超出数量', '2. Cómo se envía': '2. 发货方式', '4. Completar detalles': '4. 完善明细', ' Los m² se capturan después.': ' 平方米稍后填写。', 'Indica cómo envías cada producto: en placas, bloques de origen con su foto; en formatos y piezas, tipo de empaque y cantidad enviada.': '说明每个产品的发运方式：板材填写来源荒料及其照片；砖和件填写包装类型和发运数量。', 'Placas': '板材', 'Formatos': '砖', 'Piezas': '件', 'Este tipo se organiza por bloques. Registra cada bloque de origen y cuántas placas envías de cada uno.': '此类型按荒料组织。登记每个来源荒料以及每个荒料发运的板数。', 'Se declara por empaque. Indica el tipo de empaque y la cantidad enviada; divide por tono o lote si aplica.': '按包装申报。注明包装类型和发运数量；如适用，按色号或批次拆分。', 'Se declara por empaque. Indica el tipo de empaque y cuántos empaques envías.': '按包装申报。注明包装类型以及发运的包装数量。',
    ...
    'PI / Pedido': '形式发票 / 订单', '— automático —': '— 自动 —', 'Las PI de esta carga las asigna SOM GROUP por cada pedido.': '本批次的形式发票由SOM GROUP按每个订单分配。', 'Editar estructura': '编辑结构', 'Agregar fila': '添加行', 'Quitar fila': '删除行', 'Falta la foto de bloque: ': '缺少荒料照片: ', 'Puerto de Carga (POL)': '装货港 (POL)', 'Puerto Destino (POD)': '卸货港 (POD)', 'Agente de carga del embarque. Solo forwarders con tarifa SOM GROUP.': '本货运的货代。仅限 SOM GROUP 费率目录。', 'Solo navieras con tarifa para el forwarder elegido.': '仅限所选货代有费率的船公司。', 'Solo puertos con tarifa para el forwarder elegido.': '仅限所选货代有费率的港口。', 'Solo destinos con tarifa para la ruta elegida.': '仅限所选航线有费率的目的港。', '— selecciona naviera —': '— 选择船公司 —', '— selecciona forwarder —': '— 选择货代 —', 'Forwarder': '货代', 'Agente de carga que gestiona este embarque. Solo forwarders del catálogo SOM GROUP.': '管理本货运的货运代理。仅限 SOM GROUP 目录。', 'Compañía que opera el transporte. Solo navieras del catálogo SOM GROUP.': '运营运输的公司。仅限 SOM GROUP 目录船公司。', 'Contenedores': '集装箱', 'Contenedor': '集装箱', 'Cada caja física que viaja en el embarque. Los números son los que están pintados en el contenedor (4 letras + 7 dígitos).': '运输中的每个物理箱体。编号为集装箱上喷涂的(4 字母 + 7 数字)。', 'Captura los números de contenedor en cuanto te los entregue tu agente. Los necesitas antes del packing list.': '代理交付后请立即录入集装箱号。装箱单之前需要。', 'Agregar primer contenedor': '添加首个集装箱', 'Agregar contenedor': '添加集装箱', 'No. Contenedor': '集装箱号', 'No. de Sello': '封条号', 'Sello de seguridad que se rompe al abrir el contenedor.': '打开集装箱时会破坏的安全封条。', 'Peso bruto (kg)': '毛重 (kg)', 'Volumen (m³)': '体积 (m³)', 'No. de paquetes / bultos': '包装件数', 'Dimensión': '尺寸', 'Contenedor sin número': '无编号集装箱',
    ...
    // Onboarding
    '¡Bienvenido al portal!': '欢迎使用门户!', 'Aquí vas a registrar los datos del embarque para SOM GROUP. Te guiaremos paso a paso. No tienes que terminar de una sola vez.': '在此处录入 SOM GROUP 的货运数据。我们将逐步引导您。无需一次完成。', 'Tu progreso siempre visible': '您的进度始终可见', 'En el lado izquierdo verás el avance de cada sección con marcas visuales: verde = listo, azul = en progreso, gris = pendiente.': '左侧显示每个部分的进度: 绿色 = 完成, 蓝色 = 进行中, 灰色 = 待处理。', 'Datos generales': '一般数据', 'Embarque #1': '货运 #1', 'Embarque #2': '货运 #2', 'Ayuda contextual en cada campo': '每个字段的上下文帮助', 'Verás un ícono "?" junto a campos que pueden ser confusos. Pásale el cursor para ver una explicación con ejemplo.': '混淆字段旁会显示 "?" 图标。悬停查看解释和示例。', ' define quién paga el transporte y seguro. ': ' 定义谁支付运输和保险费。 ', 'El packing list es asistido': '装箱单为辅助式', 'En lugar de escribir cientos de líneas, te preguntaremos cuántos bloques cargas y cuántas placas tiene cada uno. Generamos las filas por ti.': '无需手写数百行,我们会询问您装载几个区块以及每个区块有几块板。我们为您生成行。', '1. Productos': '1. 产品', '2. Bloques + fotos': '2. 区块 + 照片', '3. Revisión': '3. 审核', '4. Llenar placas': '4. 填写板', '¿Listo para empezar?': '准备开始了吗?', 'Comencemos por los datos generales. Si te trabas, busca el panel de "Guía del paso actual" a la derecha — siempre te dirá qué hacer.': '从一般数据开始。如果卡住,请查看右侧的"当前步骤指南"面板 — 它会告诉您该做什么。', ' de ': ' / ',
    // Guide panel
    'Tu llenado en 4 etapas': '4 阶段填写', 'Te recomendamos seguir este orden. Si necesitas saltar a otra sección, también puedes.': '建议按此顺序进行。也可跳到其他部分。', 'Una sola vez al inicio. Identificación de la Proforma, puertos e incoterm.': '开始时一次性填写。形式发票识别、港口和贸易术语。', 'Crea uno o varios. Cada uno con logística, B/L, invoices, contenedores y packing.': '创建一个或多个。每个包含物流、提单、发票、集装箱和装箱。', 'Sube certificados de calidad y otros papeles generales.': '上传质量证书和其他一般文件。', 'Última verificación y notificación a SOM GROUP.': '最终验证并通知 SOM GROUP。', 'Esta sección define identidad y ruta. Si no sabes algo, pregunta a tu agente o déjalo vacío y vuelve después.': '此部分定义身份和路线。如不确定,请咨询代理或留空稍后填写。', 'Es el ID que tu sistema usa. Suele comenzar con "PI-".': '您系统使用的 ID。通常以 "PI-" 开头。', 'Origen y destino': '起运地和目的地', 'País y puerto de salida + puerto donde llegará.': '出发国家和港口 + 到达港口。', 'Define quién paga qué. Lo acordaste con tu contacto de SOM GROUP.': '定义谁支付什么。与 SOM GROUP 联系人约定。', 'Pagos y notas': '付款和备注', 'Términos de pago y observaciones generales.': '付款条件和一般备注。', 'Un embarque = un viaje. Puedes dividir la PO en varios embarques si la producción sale en fechas distintas.': '一个货运 = 一次航行。如生产分日期出货,可拆分为多个货运。', 'Agrega un embarque': '添加货运', 'Hazlo en cuanto tengas el buque o vuelo asignado.': '一旦分配船舶或航班立即操作。', 'Llena las 5 secciones': '填写 5 个部分', 'Logística, B/L, invoices, contenedores y packing list.': '物流、提单、发票、集装箱和装箱单。', 'Sube documentos': '上传文档', 'Certificado de origen, fitosanitario, etc.': '原产地证、植检证等。', 'Captura por pestañas': '按标签录入', 'Sigue las pestañas de izquierda a derecha. El packing list es lo más detallado — déjalo para el final.': '从左到右依次操作标签。装箱单最详细 — 留到最后。', 'Logística + B/L': '物流 + 提单', 'Naviera, buque, fechas y el documento B/L.': '船公司、船舶、日期和提单文件。', 'Invoices': '发票', 'Factura(s) comercial(es). Puede ser una global o varias parciales.': '商业发票。可为总发票或多张部分发票。', 'Los números físicos pintados en cada contenedor.': '集装箱上喷涂的实物编号。', 'Packing list': '装箱单', 'Asistente paso a paso. Captura placa por placa.': '逐步向导。逐板录入。', 'CO, fitosanitario, inspección.': '原产地证、植检、检验。', 'Documentos que aplican a toda la Proforma. Acepta PDF, JPG, PNG hasta 10 MB.': '适用于整个形式发票的文档。接受 PDF、JPG、PNG,最大 10 MB。', 'Proforma firmada': '已签署的形式发票', 'La que enviaste a SOM GROUP con firma.': '您带签名发送给 SOM GROUP 的那份。', 'Certificados de calidad': '质量证书', 'Pruebas técnicas: mineralogía, densidad, absorción.': '技术测试: 矿物学、密度、吸水率。', 'Fotos del producto': '产品照片', 'Catálogo o muestras a granel.': '目录或散装样品。', 'Verifica todo': '全部验证', 'Una vez marcada como completa, SOM GROUP recibe una notificación. Si después necesitas editar, pídeselo a tu contacto.': '标记为完成后,SOM GROUP 将收到通知。如需后续编辑,请联系您的联系人。', 'Datos clave que se enviarán.': '将发送的关键数据。', 'Checklist por sección': '按部分的检查清单', 'Si algo está en azul, vuelve a esa sección.': '如有蓝色项目,请返回该部分。', 'Marcar como completa': '标记为完成', 'Solo se habilita cuando todo está en verde.': '仅当所有项目为绿色时启用。', 'mapa de ruta': '路线图', 'ilustración guía': '指南插图',
    // Sidebar
    'Todo listo': '全部就绪', 'En proceso': '进行中', '% completado': '% 完成', 'PI sin número': 'PI 无编号',
  },
  it: {
    /* i18n-pl-lock */ 'Packing list cerrado': 'Packing list bloccata', 'Este embarque ya fue recibido en tránsito': 'Questa spedizione è già stata ricevuta in transito', 'Esta proforma ya fue marcada como completa.': 'Questa proforma è già stata segnata come completa.', 'El packing list ya no se puede modificar. Si necesitas corregirlo, pídeselo a tu contacto en SOM GROUP.': 'La packing list non può più essere modificata. Se serve una correzione, chiedila al tuo contatto in SOM GROUP.', 'Recibido': 'Ricevuto', 'Ocultar detalle': 'Nascondi dettaglio',
    /* i18n-pl-v2 */ 'rellenar valor': 'inserisci valore', 'Empaque': 'Imballaggio', 'm² (por capturar)': 'm² (da inserire)', 'Cantidad': 'Quantità', 'No. Palet': 'N. Pallet', 'No. Tarima': 'N. Pedana', 'No. Caja': 'N. Scatola', 'No. Empaque': 'N. Imballo', 'No. Paquete': 'N. Pacco', 'No. Pieza': 'N. Pezzo', 'Área m²': 'Area m²', 'Tono/Lote': 'Tono/Lotto', 'Lote': 'Lotto', 'Placa #': 'Lastra #', 'Largo': 'Lunghezza', 'Ancho': 'Larghezza', 'Estado': 'Stato', 'PALET': 'PALLET', 'CAJA': 'SCATOLA', 'SUELTO': 'SFUSO', 'TARIMA': 'PEDANA', 'Pendientes de detalle': 'In attesa di dettaglio', 'Completo': 'Completo', 'Pendiente de detalle': 'In attesa di dettaglio', 'm² pendiente': 'm² in attesa', 'cantidad pendiente': 'quantità in attesa', 'Falta empaque': 'Manca imballaggio', 'Listo': 'Pronto', 'Incompleto': 'Incompleto', 'Foto opcional': 'Foto opzionale', 'Sin foto': 'Senza foto', 'Pendiente m²': 'm² in attesa', '¿Cómo viene empacado?': 'Come è imballato?', '— elige —': '— scegli —', '(vacías)': '(vuote)', 'Proforma (PI)': 'Proforma (PI)', 'Por proforma': 'Per proforma', 'Suelto': 'Sfuso', 'Caja': 'Scatola', 'Palet': 'Pallet', 'Tarima': 'Pedana', 'Cantidad total de metros cuadrados': 'Metri quadri totali', 'Cantidad total': 'Quantità totale', 'Dividir por tono/lote': 'Dividi per tono/lotto', ' Los metros cuadrados se capturan después.': ' I metri quadri si inseriscono dopo.', 'Placa': 'Lastra', 'Formato': 'Formato', 'Pieza': 'Pezzo', 'Llenar detalle pendiente': 'Compila dettaglio', 'Llena el detalle pendiente': 'Compila il dettaglio', 'Crear y terminar': 'Crea e termina', 'Llenar detalle (': 'Compila dettaglio (', ' fila)': ' riga)', ' filas)': ' righe)', 'Crear ': 'Crea ', ' filas y terminar': ' righe e termina', 'Cada placa y cada empaque (palet/caja) es una fila editable.': 'Ogni lastra e ogni imballo (pallet/scatola) è una riga modificabile.', ' filas completas': ' righe complete', 'No hay filas que detallar todavía.': 'Nessuna riga da dettagliare ancora.', 'Ver detalle': 'Vedi dettaglio', 'Ocultar': 'Nascondi', 'Aún no se han generado filas para este packing.': 'Nessuna riga generata per questo packing.', 'Subir/Reemplazar foto': 'Carica/Sostituisci foto', 'Subir/Reemplazar foto de la placa': 'Carica/Sostituisci foto della lastra', 'Plataforma': 'Piattaforma', '— sin asignar —': '— non assegnato —', 'plataforma / camión': 'piattaforma / camion', 'Agregar Bloque': 'Aggiungi blocco', 'Organiza el contenido del embarque': 'Organizza il contenuto della spedizione', 'El tipo lo define la categoría del producto. En placas, arma los bloques y sube una foto de cada uno. En formatos y piezas, primero indica cómo viene empacado (suelto, caja o palet) y luego la cantidad.': 'Il tipo è definito dalla categoria del prodotto. Per le lastre, crea i blocchi e carica una foto di ciascuno. Per formati e pezzi, indica prima come è imballato (sfuso, scatola o pallet) e poi la quantità.', 'Captura el detalle de cada fila: una placa o un empaque (palet/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades, y agregas contenedor, fotos y notas.': 'Inserisci il dettaglio di ogni riga: una lastra o un imballo (pallet/scatola) per riga. Qui regoli dimensioni, m² e quantità, e aggiungi container, foto e note.', /* i18n-declaracion */ 'Cómo se envía': 'Come viene spedito', 'Completar detalles': 'Completa i dettagli', 'Para empezar, ¿qué productos vas a enviar?': 'Per iniziare, quali prodotti spedirai?', 'Declara cómo viene organizado el envío': 'Dichiara come è organizzata la spedizione', 'Revisa la declaración del envío': 'Rivedi la dichiarazione di spedizione', 'Completa los detalles del envío': 'Completa i dettagli della spedizione', 'Indica cómo estás enviando cada producto. Para placas, registra los bloques de origen y sube una foto de cada bloque. Para formatos y piezas, indica el tipo de empaque, la cantidad enviada y, cuando aplique, sube una foto como evidencia del envío.': 'Indica come stai spedendo ogni prodotto. Per le lastre, registra i blocchi di origine e carica una foto di ogni blocco. Per formati e pezzi, indica il tipo di imballaggio, la quantità spedita e, quando applicabile, carica una foto come prova della spedizione.', 'Confirma lo que declaras enviar en este packing list. Si algo no cuadra, regresa al paso anterior.': 'Conferma ciò che dichiari di spedire in questo packing list. Se qualcosa non torna, torna al passo precedente.', 'Detalla lo que envías: una placa o un empaque (palet/tarima/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades enviadas, y agregas contenedor, fotos y notas.': 'Dettaglia ciò che spedisci: una lastra o un imballo (pallet/pedana/scatola) per riga. Qui regoli dimensioni, m² e quantità spedite, e aggiungi container, foto e note.', 'Revisar declaración del envío': 'Rivedi dichiarazione di spedizione', 'Ajustar declaración': 'Modifica dichiarazione', 'Completar detalles (': 'Completa dettagli (', 'Enviar declaración al comprador': 'Invia la dichiarazione al compratore', 'Guardar declaración y volver al embarque': 'Salva la dichiarazione e torna alla spedizione', 'Declarado': 'Dichiarato', 'Declaración incompleta': 'Dichiarazione incompleta', 'Falta foto requerida': 'Manca foto richiesta', 'Falta cantidad enviada': 'Manca quantità spedita', 'Falta indicar empaque': 'Imballaggio non indicato', 'Falta agregar bloque': 'Manca il blocco', 'Falta declarar placas': 'Lastre non dichiarate', 'Falta foto del bloque': 'Manca foto del blocco', 'Cantidad a enviar: ': 'Quantità da spedire: ', ' · Declarado: ': ' · Dichiarato: ', '¿Cómo se envía este producto?': 'Come viene spedito questo prodotto?', 'Tipo de empaque': 'Tipo di imballaggio', 'Metros cuadrados enviados (total)': 'Metri quadri spediti (totale)', 'Cantidad total enviada': 'Quantità totale spedita', 'Placas enviadas': 'Lastre spedite', 'Agregar empaque': 'Aggiungi imballo', 'Evidencia opcional del envío': 'Prova facoltativa della spedizione', 'Sin evidencia requerida': 'Nessuna prova richiesta', 'Foto del bloque': 'Foto del blocco', 'Evidencia (opc.)': 'Prova (fac.)', 'Filas de la declaración': 'Righe della dichiarazione', 'Con detalles pendientes': 'Con dettagli in sospeso', 'Resumen de la declaración': 'Riepilogo della dichiarazione', 'Puedes continuar y subirlas después, pero la declaración del envío no se considerará completa hasta que cada bloque tenga al menos una foto.': 'Puoi continuare e caricarle dopo, ma la dichiarazione di spedizione non sarà considerata completa finché ogni blocco non avrà almeno una foto.', 'Cantidad excedente': 'Quantità in eccesso', 'Declaraste ': 'Hai dichiarato ', ' de ': ' di ', ' placas. Faltan ': ' lastre. Mancano ', ' placas por declarar.': ' lastre da dichiarare.', ' placas. Hay ': ' lastre. Ci sono ', ' placas excedentes. Confirma si deseas continuar con esta diferencia.': ' lastre in eccesso. Conferma se vuoi continuare con questa differenza.', ' unidades. Faltan ': ' unità. Mancano ', ' unidades por declarar.': ' unità da dichiarare.', ' unidades. Hay ': ' unità. Ci sono ', ' unidades excedentes. Confirma si deseas continuar con esta diferencia.': ' unità in eccesso. Conferma se vuoi continuare con questa differenza.', 'Confirmo enviar la cantidad excedente': 'Confermo di spedire la quantità in eccesso', '2. Cómo se envía': '2. Come viene spedito', '4. Completar detalles': '4. Completa i dettagli', ' Los m² se capturan después.': ' I m² si inseriscono dopo.', 'Indica cómo envías cada producto: en placas, bloques de origen con su foto; en formatos y piezas, tipo de empaque y cantidad enviada.': 'Indica come spedisci ogni prodotto: per le lastre, blocchi di origine con foto; per formati e pezzi, tipo di imballaggio e quantità spedita.', 'Placas': 'Lastre', 'Formatos': 'Formati', 'Piezas': 'Pezzi', 'Este tipo se organiza por bloques. Registra cada bloque de origen y cuántas placas envías de cada uno.': 'Questo tipo si organizza per blocchi. Registra ogni blocco di origine e quante lastre spedisci da ciascuno.', 'Se declara por empaque. Indica el tipo de empaque y la cantidad enviada; divide por tono o lote si aplica.': 'Si dichiara per imballo. Indica il tipo di imballaggio e la quantità spedita; dividi per tono o lotto se applicabile.', 'Se declara por empaque. Indica el tipo de empaque y cuántos empaques envías.': 'Si dichiara per imballo. Indica il tipo di imballaggio e quanti imballi spedisci.',
    ...
    'PI / Pedido': 'PI / Ordine', '— automático —': '— automatico —', 'Las PI de esta carga las asigna SOM GROUP por cada pedido.': 'Le PI di questo carico sono assegnate da SOM GROUP per ogni ordine.', 'Editar estructura': 'Modifica struttura', 'Agregar fila': 'Aggiungi riga', 'Quitar fila': 'Rimuovi riga', 'Falta la foto de bloque: ': 'Manca la foto del blocco: ', 'Puerto de Carga (POL)': 'Porto di Carico (POL)', 'Puerto Destino (POD)': 'Porto di Destinazione (POD)', 'Agente de carga del embarque. Solo forwarders con tarifa SOM GROUP.': 'Spedizioniere del carico. Solo catalogo tariffe SOM GROUP.', 'Solo navieras con tarifa para el forwarder elegido.': 'Solo compagnie con tariffa per il forwarder scelto.', 'Solo puertos con tarifa para el forwarder elegido.': 'Solo porti con tariffa per il forwarder scelto.', 'Solo destinos con tarifa para la ruta elegida.': 'Solo destinazioni con tariffa per la rotta scelta.', '— selecciona naviera —': '— seleziona compagnia —', '— selecciona forwarder —': '— seleziona forwarder —', 'Forwarder': 'Forwarder', 'Agente de carga que gestiona este embarque. Solo forwarders del catálogo SOM GROUP.': 'Spedizioniere di questo carico. Solo catalogo SOM GROUP.', 'Compañía que opera el transporte. Solo navieras del catálogo SOM GROUP.': 'Compagnia di trasporto. Solo catalogo SOM GROUP.', 'Contenedores': 'Container', 'Contenedor': 'Container', 'Cada caja física que viaja en el embarque. Los números son los que están pintados en el contenedor (4 letras + 7 dígitos).': 'Ogni cassa fisica nella spedizione. I numeri sono quelli dipinti sul container (4 lettere + 7 cifre).', 'Captura los números de contenedor en cuanto te los entregue tu agente. Los necesitas antes del packing list.': 'Inserisci i numeri dei container appena il tuo agente te li dà. Servono prima del packing list.', 'Agregar primer contenedor': 'Aggiungi primo container', 'Agregar contenedor': 'Aggiungi container', 'No. Contenedor': 'N. Container', 'No. de Sello': 'N. Sigillo', 'Sello de seguridad que se rompe al abrir el contenedor.': 'Sigillo di sicurezza che si rompe aprendo il container.', 'Peso bruto (kg)': 'Peso lordo (kg)', 'Volumen (m³)': 'Volume (m³)', 'No. de paquetes / bultos': 'N. colli / pacchi', 'Dimensión': 'Dimensione', 'Contenedor sin número': 'Container senza numero',
  ...
  pt: {
    ...
    'PI / Pedido': 'PI / Pedido', '— automático —': '— automático —', 'Las PI de esta carga las asigna SOM GROUP por cada pedido.': 'As PIs desta carga são atribuídas pela SOM GROUP para cada pedido.', 'Editar estructura': 'Editar estrutura', 'Agregar fila': 'Adicionar linha', 'Quitar fila': 'Remover linha', 'Falta la foto de bloque: ': 'Falta a foto do bloco: ', 'Puerto de Carga (POL)': 'Porto de Embarque (POL)', 'Puerto Destino (POD)': 'Porto de Destino (POD)', 'Agente de carga del embarque. Solo forwarders con tarifa SOM GROUP.': 'Agente de carga do embarque. Somente catálogo de tarifas SOM GROUP.', 'Solo navieras con tarifa para el forwarder elegido.': 'Somente armadores com tarifa para o forwarder escolhido.', 'Solo puertos con tarifa para el forwarder elegido.': 'Somente portos com tarifa para o forwarder escolhido.', 'Solo destinos con tarifa para la ruta elegida.': 'Somente destinos com tarifa para a rota escolhida.', '— selecione armador —': '— selecione armador —', '— selecciona forwarder —': '— selecione forwarder —', 'Forwarder': 'Forwarder', 'Agente de carga que gestiona este embarque. Solo forwarders del catálogo SOM GROUP.': 'Agente de carga deste embarque. Somente catálogo SOM GROUP.', 'Compañía que opera el transporte. Solo navieras del catálogo SOM GROUP.': 'Companhia de transporte. Somente armadores do catálogo SOM GROUP.', 'Contenedores': 'Contêineres', 'Contenedor': 'Contêiner', 'Cada caja física que viaja en el embarque. Los números son los que están pintados en el contenedor (4 letras + 7 dígitos).': 'Cada caixa física que viaja no embarque. Os números são os pintados no contêiner (4 letras + 7 dígitos).', 'Captura los números de contenedor en cuanto te los entregue tu agente. Los necesitas antes del packing list.': 'Capture os números de contêiner assim que seu agente os entregar. Você precisa deles antes do packing list.', 'Agregar primer contenedor': 'Adicionar primeiro contêiner', 'Agregar contenedor': 'Adicionar contêiner', 'No. Contenedor': 'Nº Contêiner', 'No. de Sello': 'Nº Lacre', 'Sello de seguridad que se rompe al abrir el contenedor.': 'Lacre de segurança que se rompe ao abrir o contêiner.', 'Peso bruto (kg)': 'Peso bruto (kg)', 'Volumen (m³)': 'Volume (m³)', 'No. de paquetes / bultos': 'Nº de pacotes / volumes', 'Dimensión': 'Dimensão', 'Contenedor sin número': 'Contêiner sem número',
        ...
        React.createElement("div", { className: "progress-card" },
            ...
            React.createElement("div", { className: "progress-info" },
                ...
                React.createElement("span", { className: "value" },
                    status.overall,
                    "% completado"),
                React.createElement("span", { className: "meta" }, window.PORTAL_PI_LABEL(proforma.globals.proforma_number || 'PI sin número')))),
            ...
            React.createElement("div", { className: "fld-row" },
                (window.PORTAL_CARGO
                    ...
                    : React.createElement(Field, { label: "N\u00FAmero de Proforma", required: true, help: "Es el n\u00FAmero con el que tu sistema identifica esta venta (Proforma Invoice).", helpExample: "Ej: PI-9920-A", error: validationStyle !== 'block' && errors.proforma_number },
                        React.createElement(Input, { mono: true, placeholder: "Escribir aquí", value: g.proforma_number, onChange: (e) => update('proforma_number', e.target.value) }))),
                React.createElement(Field, { label: "Factura global", optional: true, help: "Si emites una factura comercial que cubre toda la PO, escr\u00EDbela aqu\u00ED. Si tienes una por embarque, d\u00E9jalo vac\u00EDo y ll\u00E9nalo en cada embarque." },
                    React.createElement(Input, { mono: true, placeholder: "Escribir aquí (opcional)", value: g.invoice_global, onChange: (e) => update('invoice_global', e.target.value) })))),
        React.createElement("div", { className: "card" },
            React.createElement("div", { className: "card-head" },
                React.createElement("div", null,
            ...
            React.createElement(Field, { optional: true, hint: "Esto se incluir\u00E1 en la confirmaci\u00F3n final. Puedes dejarlo vac\u00EDo si no aplica." },
                React.createElement(Textarea, { rows: 3, placeholder: "Escribir aquí", value: g.general_notes, onChange: (e) => update('general_notes', e.target.value) }))),
        ...
        React.createElement("div", { className: "page-head" },
            ...
            React.createElement("div", { className: "head-actions" },
                React.createElement("span", { className: "text-muted text-small" },
                    sst.pct,
                    "% completo"),
                !ship.is_locked && React.createElement(Btn, { variant: "ghost", icon: "trash", className: "btn-danger-ghost", onClick: () => {
...
const TabLogistics = ({ ship, updateShip }) => (React.createElement("div", null,
    React.createElement("div", { className: "card" },
        ...
        React.createElement("div", { className: "fld-row cols-3" },
            ...
            (() => {
                // ── Cascada del TARIFARIO: forwarder → naviera → POL → POD ──
                // El país ya viene de la OC (el proveedor no lo elige). Cada
                // selector solo ofrece combinaciones CON TARIFA ACTIVA; si el
                // forwarder tiene un solo puerto, esa es la única opción.
                const ROUTES = PORTAL_RAW_PAYLOAD.tariff_routes || [];
                const uniq = (arr, idKey, nameKey) => {
                    const seen = new Map();
                    arr.forEach(r => { if (r[idKey] && !seen.has(r[idKey])) seen.set(r[idKey], r[nameKey]); });
                    return [...seen.entries()].map(([id, name]) => ({ id, name }));
                };
                const byFwd = ship.forwarder_id ? ROUTES.filter(r => r.forwarder_id === ship.forwarder_id) : ROUTES;
                const byNav = ship.naviera_id ? byFwd.filter(r => r.naviera_id === ship.naviera_id) : byFwd;
                const byPol = ship.pol_id ? byNav.filter(r => r.pol_id === ship.pol_id) : byNav;
                const fwdOpts = uniq(ROUTES, 'forwarder_id', 'forwarder_name');
                const navOpts = uniq(byFwd, 'naviera_id', 'naviera_name');
                const polOpts = uniq(byNav, 'pol_id', 'pol_name');
                const podOpts = uniq(byPol, 'pod_id', 'pod_name');
                const pickCascade = (patch2) => {
                    // Al cambiar un nivel se limpian los inferiores inválidos y,
                    // si un nivel queda con UNA sola opción, se autoselecciona.
                    const next = { ...ship, ...patch2 };
                    const f1 = next.forwarder_id ? ROUTES.filter(r => r.forwarder_id === next.forwarder_id) : ROUTES;
                    if (next.naviera_id && !f1.some(r => r.naviera_id === next.naviera_id)) { next.naviera_id = false; next.shipping_line = ''; }
                    const f2 = next.naviera_id ? f1.filter(r => r.naviera_id === next.naviera_id) : f1;
                    if (next.pol_id && !f2.some(r => r.pol_id === next.pol_id)) { next.pol_id = false; next.port_origin = ''; }
                    const f3 = next.pol_id ? f2.filter(r => r.pol_id === next.pol_id) : f2;
                    if (next.pod_id && !f3.some(r => r.pod_id === next.pod_id)) { next.pod_id = false; next.port_destination = ''; }
                    const autoNav = uniq(f1, 'naviera_id', 'naviera_name');
                    if (!next.naviera_id && autoNav.length === 1) { next.naviera_id = autoNav[0].id; next.shipping_line = autoNav[0].name; }
                    const f2b = next.naviera_id ? f1.filter(r => r.naviera_id === next.naviera_id) : f1;
                    const autoPol = uniq(f2b, 'pol_id', 'pol_name');
                    if (!next.pol_id && autoPol.length === 1) { next.pol_id = autoPol[0].id; next.port_origin = autoPol[0].name; }
                    const f3b = next.pol_id ? f2b.filter(r => r.pol_id === next.pol_id) : f2b;
                    const autoPod = uniq(f3b, 'pod_id', 'pod_name');
                    if (!next.pod_id && autoPod.length === 1) { next.pod_id = autoPod[0].id; next.port_destination = autoPod[0].name; }
                    updateShip({
                        forwarder_id: next.forwarder_id, naviera_id: next.naviera_id,
                        shipping_line: next.shipping_line, pol_id: next.pol_id,
                        port_origin: next.port_origin, pod_id: next.pod_id,
                        port_destination: next.port_destination,
                    });
                };
                if (!ROUTES.length) {
                    return React.createElement(Field, { label: "Naviera / Aerol\u00EDnea", required: true, help: "Compa\u00F1\u00EDa que opera el transporte.", helpExample: "COSCO, MSC, Hapag-Lloyd\u2026" },
                        React.createElement(Input, { placeholder: "Escribir aquí", value: ship.shipping_line, onChange: (e) => updateShip({ shipping_line: e.target.value }) }));
                }
                return React.createElement(React.Fragment, null,
                    React.createElement(Field, { label: "Forwarder", required: true, help: "Agente de carga del embarque. Solo forwarders con tarifa SOM GROUP." },
                        React.createElement(Select, { value: ship.forwarder_id || '', onChange: (e) => pickCascade({ forwarder_id: e.target.value ? parseInt(e.target.value, 10) : false }) },
                            React.createElement("option", { value: "" }, "Selecciona\u2026"),
                            fwdOpts.map(o => React.createElement("option", { key: o.id, value: o.id }, o.name)))),
                    React.createElement(Field, { label: "Naviera / Aerol\u00EDnea", required: true, help: "Solo navieras con tarifa para el forwarder elegido." },
                        React.createElement(Select, { value: ship.naviera_id || '', disabled: !ship.forwarder_id, onChange: (e) => {
                                const nid = e.target.value ? parseInt(e.target.value, 10) : false;
                                const nav = navOpts.find(n => n.id === nid);
                                pickCascade({ naviera_id: nid, shipping_line: nav ? nav.name : '' });
                            } },
                            React.createElement("option", { value: "" }, "Selecciona\u2026"),
                            navOpts.map(o => React.createElement("option", { key: o.id, value: o.id }, o.name)))),
                    React.createElement(Field, { label: "Puerto de Carga (POL)", required: true, help: "Solo puertos con tarifa para el forwarder elegido." },
                        React.createElement(Select, { value: ship.pol_id || '', disabled: !ship.forwarder_id, onChange: (e) => {
                                const pid = e.target.value ? parseInt(e.target.value, 10) : false;
                                const pol = polOpts.find(x => x.id === pid);
                                pickCascade({ pol_id: pid, port_origin: pol ? pol.name : '' });
                            } },
                            React.createElement("option", { value: "" }, "Selecciona\u2026"),
                            polOpts.map(o => React.createElement("option", { key: o.id, value: o.id }, o.name)))),
                    React.createElement(Field, { label: "Puerto Destino (POD)", required: true, help: "Solo destinos con tarifa para la ruta elegida." },
                        React.createElement(Select, { value: ship.pod_id || '', disabled: !ship.forwarder_id, onChange: (e) => {
                                const pid = e.target.value ? parseInt(e.target.value, 10) : false;
                                const pod = podOpts.find(x => x.id === pid);
                                pickCascade({ pod_id: pid, port_destination: pod ? pod.name : '' });
                            } },
                            React.createElement("option", { value: "" }, "Selecciona\u2026"),
                            podOpts.map(o => React.createElement("option", { key: o.id, value: o.id }, o.name)))));
            })(),
    ...
    React.createElement("div", { className: "card" },
        ...
        React.createElement("div", { className: "fld-row cols-2" },
            React.createElement(Field, { label: "N\u00FAmero de B/L", required: true, help: "El n\u00FAmero \u00FAnico que asigna la naviera a tu embarque.", helpExample: "COSU6817042500" },
                React.createElement(Input, { mono: true, placeholder: "Escribir aquí", value: ship.bl_number, onChange: (e) => updateShip({ bl_number: e.target.value }) })),
                ...
                ship.invoices.map((inv, i) => (React.createElement("div", { key: inv.id, style: { border: '1px solid var(--border)', borderRadius: 12, padding: 16, background: 'var(--surface-alt)' } },
                    ...
                    React.createElement("div", { className: "fld-row cols-3" },
                        React.createElement(Field, { label: "No. Invoice", required: true },
                            React.createElement(Input, { mono: true, placeholder: "Escribir aquí", value: inv.number, onChange: (e) => updInv(inv.id, { number: e.target.value }) })),
            ...
            return (React.createElement("div", { key: c.id, style: {
                ...
                React.createElement("div", { className: "fld-row cols-3" },
                    React.createElement(Field, { label: "No. Contenedor", required: true, help: "4 letras (c\u00F3digo de naviera) + 7 d\u00EDgitos. Est\u00E1 pintado en grande en el costado del contenedor.", helpExample: "COSU6817042", error: isBad ? 'Formato: 4 letras + 7 dígitos (ej. COSU6817042)' : null },
                        React.createElement(Input, { mono: true, placeholder: "Escribir aquí", value: c.number, onChange: (e) => updC(c.id, { number: e.target.value.toUpperCase() }) })),
                    React.createElement(Field, { label: "No. de Sello", required: true, help: "Sello de seguridad que se rompe al abrir el contenedor." },
                        React.createElement(Input, { mono: true, placeholder: "Escribir aquí", value: c.seal, onChange: (e) => updC(c.id, { seal: e.target.value.toUpperCase() }) })),
                ...
                React.createElement("div", { className: "fld-row cols-3", style: { marginTop: 14 } },
                    React.createElement(Field, { label: "Peso bruto (kg)" },
                        React.createElement(Input, { mono: true, type: "number", placeholder: "0", value: c.weight || '', onChange: (e) => updC(c.id, { weight: +e.target.value }) })),
                    React.createElement(Field, { label: "Volumen (m\u00B3)" },
                        React.createElement(Input, { mono: true, type: "number", step: "0.1", placeholder: "0.0", value: c.volume || '', onChange: (e) => updC(c.id, { volume: +e.target.value }) })),
                    React.createElement(Field, { label: "No. de paquetes / bultos" },
                        React.createElement(Input, { mono: true, type: "number", placeholder: "0", value: c.packages || '', onChange: (e) => updC(c.id, { packages: +e.target.value }) })))));
        })))));
};
/* ============================================================
...
const PL_PKG_NUM_LABEL = (rows) => {
    ...
    const hasT = kinds.has('tarima');
    if (hasP && !hasC && !hasT) return 'No. Palet';
    if (hasT && !hasP && !hasC) return 'No. Tarima';
    if (hasC && !hasP && !hasT) return 'No. Caja';
    if (hasP || hasC || hasT) return 'No. Empaque';
...
const PL_STATE = (r) => {
    ...
    if (kind === 'placa') {
        // La foto NO es obligatoria: una placa con medidas y contenedor ya
        // está declarada. La foto suma si existe, pero no bloquea el
        // "completado" (antes atoraba TODO en pendiente).
        const ok = (parseFloat(r.h) > 0) && (parseFloat(r.w) > 0) && !!r.container;
        return ok ? { tone: 'done', text: 'Declarado', icon: 'check' } : { tone: 'todo', text: 'Declaración incompleta' };
    }
    if (kind === 'formato' && !PL_LOOSE(pkg)) {
        // Formato empacado: los m² quedan pendientes hasta capturarse.
        return (parseFloat(r.quantity) > 0) ? { tone: 'done', text: 'Declarado', icon: 'check' } : { tone: 'partial', text: 'Falta cantidad enviada' };
    }
    // Formato suelto / Pieza: completo en cuanto hay cantidad.
    return (parseFloat(r.quantity) > 0) ? { tone: 'done', text: 'Declarado', icon: 'check' } : { tone: 'todo', text: 'Falta cantidad enviada' };
};
    ...
    return React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 14 } }, groups.map(g => {
        ...
        if (kind === 'placa') {
            ...
            table = React.createElement("table", { className: "sheet-table" },
                React.createElement("thead", null, React.createElement("tr", null,
                    React.createElement("th", { style: { width: 28 } }, "#"),
                    React.createElement("th", null, "Bloque/Tono"),
...
const TabPackings = ({ ship, updateShip, openPackingWizard, proforma, onDeletePacking }) => {
    ...
    const locked = !!ship.is_locked;
    const lockedPickings = (ship.received_pickings || []).join(', ');
    return (React.createElement("div", null,
        locked && React.createElement(Callout, { tone: "warn", icon: "alert", title: tr('Packing list cerrado') },
            lockedPickings
                ? tr('Este embarque ya fue recibido en tránsito') + ' (' + lockedPickings + '). '
                : tr('Esta proforma ya fue marcada como completa.') + ' ',
            tr('El packing list ya no se puede modificar. Si necesitas corregirlo, pídeselo a tu contacto en SOM GROUP.')),
        React.createElement("div", { className: "card" },
            React.createElement("div", { className: "card-head" },
                React.createElement("div", null,
                ...
                locked
                    ? React.createElement(Badge, { tone: "done" }, React.createElement(Icon, { name: "check", size: 10 }), " ", tr('Recibido'))
            ...
            ship.packings.length === 0 ? (React.createElement(Empty, { icon: "box", title: "Sin packing lists todav\u00EDa" },
                ...
                return (React.createElement("div", { key: pk.id, style: { display: 'flex', flexDirection: 'column' } },
                    ...
                    React.createElement("div", { style: { flex: 1 } },
                        ...
                        !photosOk && React.createElement("div", { className: "text-muted", style: { fontSize: 12.5 } },
                            React.createElement("span", { style: { color: 'var(--warn)' } },
                    ...
                    locked
                        ? React.createElement("div", { style: { display: 'flex', gap: 8 } },
                            React.createElement(Btn, { variant: "secondary", icon: "eye", onClick: () => setOpenId(isOpen ? null : pk.id) }, isOpen ? tr('Ocultar detalle') : tr('Ver detalle')))
                        : React.createElement("div", { style: { display: 'flex', gap: 8 } },
...
const WIZARD_STEPS = [
    ...
    { id: 2, label: 'Cómo se envía' },
    { id: 3, label: 'Revisión' },
    { id: 4, label: 'Completar detalles' },
];
...
const GROUP_MODES = {
    ...
    formato: { term: FORMATO_TERM, icon: 'image', color: '#5b6ee1',       photo: 'optional', generatesRows: false, explainer: 'brief', countLabel: 'Cantidad total de metros cuadrados' },
...
const PL_ROW_KEEP = ['h', 'w', 'thickness', 'container', 'container_id', 'pi_header_id', 'notes', 'photo', 'image_preview', 'quantity', 'weight', 'plate', 'atado', 'pedimento', 'ref', '_odoo_id', '_client_id'];
    ...
    (draft.blocks || []).forEach((b, bIdx) => {
        ...
        if (mode === 'placa') {
            for (let i = 0; i < (+b.count || 0); i++) {
                ...
                // PI y contenedor del BLOQUE precargan las filas (cargo).
...
const PL_EMP_UNIT = (kind, n) => (kind === 'caja') ? (n === 1 ? 'caja' : 'cajas') : (kind === 'tarima') ? (n === 1 ? 'tarima' : 'tarimas') : (n === 1 ? 'palet' : 'palets');
const PL_PRODUCT_SUMMARY = (prows) => {
    ...
    if (kind === 'placa') {
        ...
        return { kind, parts: [`${placas} placa${placas === 1 ? '' : 's'}`, allDone ? 'declaradas' : 'con detalles pendientes'], tone: allDone ? 'done' : 'partial' };
    }
    ...
    if (loose.length) {
        ...
        parts.push(kind === 'formato' ? `${sum} m²` : `${sum} unidad${sum === 1 ? '' : 'es'}`, 'suelto', sum > 0 ? 'declarado' : 'falta cantidad enviada');
    ...
    if (packed.length) {
        ...
        if (kind === 'formato') {
            parts.push(`${packed.length} ${unit}`, captured > 0 ? `${captured} m²` : 'm² por declarar');
            if (captured <= 0) tone = 'partial';
        } else {
            parts.push(`${packed.length} ${unit}`, captured > 0 ? `${captured} piezas` : 'cantidad por declarar');
            if (captured <= 0) tone = 'partial';
        }
    }
...
const PackingWizard = ({ proforma, shipmentId, packingId, startAtStructure, onClose, onSave, sampleRows, pendingImages }) => {
    ...
    const s2GroupOk = (g, t) => {
        // Multi-proforma (cargo): la PI del bloque es OBLIGATORIA — define a
        // qué proforma pertenece lo que se envía y precarga las filas.
        if (window.PORTAL_CARGO && !g.pi_header_id)
            return false;
        if (t === 'placa')
    ...
    const canNext = () => {
        ...
        if (step === 2) {
            if (draft.blocks.length === 0)
                return false;
            const cfgOk = draft.blocks.every(b => {
                const mode = groupModeById(draft, proforma.products, b.product);
                if (mode === 'placa')
            ...
            if (!cfgOk)
                return false;
            // Excedente: declarar MÁS de lo solicitado exige confirmación explícita.
            return (draft.products || []).every(pid => {
                const p = proforma.products.find(pp => String(pp.id) === String(pid)) || {};
                ...
                if (mode === 'pieza' && pgroups.length && pgroups.every(g => (g.packaging || {}).kind === 'suelto'))
                    ...
                    return true;
                return declared <= req || !!((draft.over_ok || {})[pid]);
            });
        }
        return true;
    };
    return (React.createElement("div", { className: "modal-scrim", onClick: (e) => e.target === e.currentTarget && commitAndClose() },
        ...
        React.createElement("div", { className: "modal modal-wide" + (step === 4 ? " modal-step4" : ""), style: { maxWidth: 1280 } },
            React.createElement("div", { className: "modal-head" },
                React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 600, color: 'var(--ink-4)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 } },
                        ...
                        " packing list"),
                    React.createElement("h2", null, step === 1 ? 'Para empezar, ¿qué productos vas a enviar?' :
                        step === 2 ? 'Declara cómo viene organizado el envío' :
                            step === 3 ? 'Revisa la declaración del envío' :
                                'Completa los detalles del envío'),
                    React.createElement("p", { className: "sub" },
                        step === 1 && 'Selecciona uno o más productos de la PO. Cada packing list puede incluir varios productos.',
                        step === 2 && 'Indica cómo envías cada producto: en placas, bloques de origen con su foto; en formatos y piezas, tipo de empaque y cantidad enviada.',
                        step === 3 && 'Confirma lo que declaras enviar en este packing list. Si algo no cuadra, regresa al paso anterior.',
                        step === 4 && 'Detalla lo que envías: una placa o un empaque (palet/tarima/caja) por renglón. Aquí ajustas dimensiones, m² y cantidades enviadas, y agregas contenedor, fotos y notas.')),
            ...
            React.createElement("div", { className: "modal-body", style: { background: step === 4 ? 'var(--bg)' : 'var(--surface)' } },
                ...
                step === 2 && React.createElement(Step2Blocks, { proforma: proforma, draft: draft, setDraft: setDraft, pendingImages: pendingImages, typeTab: s2Active, ship: ship }),
            ...
            React.createElement("div", { className: "modal-foot" },
                React.createElement("div", { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
                    ...
                    step === 2 && s2Types.length > 1 && s2Types.map(t => {
                        ...
                        return React.createElement("button", { key: t, type: "button", onClick: () => setS2Tab(t), style: { display: 'flex', alignItems: 'center', gap: 7, padding: '7px 12px', borderRadius: 10, cursor: 'pointer', border: active ? `1.5px solid ${cfgT.color}` : '1px solid var(--border)', background: active ? 'var(--accent-soft)' : 'var(--surface)', fontWeight: active ? 700 : 500, fontSize: 12.5 } },
                            ...
                            S2_LABELS[t],
                            React.createElement("span", { title: ready ? 'Declarado' : 'Declaración incompleta', style: { width: 8, height: 8, borderRadius: 99, background: ready ? 'var(--ok)' : 'var(--warn)' } }));
                    })),
                React.createElement("div", { style: { display: 'flex', gap: 8, alignItems: 'center' } },
                    ...
                    step === 3 && (React.createElement(React.Fragment, null,
                        React.createElement(Btn, { variant: "ghost", onClick: () => setStep(2) }, "Ajustar declaración"),
                        ...
                        (totalRowsToGen > 0
                            ...
                            : React.createElement(Btn, { variant: "accent", icon: "check", onClick: commitAndClose }, "Enviar declaración al comprador")))),
                    step === 4 && (React.createElement(Btn, { variant: "primary", icon: "check", onClick: commitAndClose }, "Guardar declaración y volver al embarque")))))));
};
/* ====================== Step 1 ====================== */
...
/* ====================== Step 2 ====================== */
const Step2Blocks = ({ proforma, draft, setDraft, pendingImages, typeTab, ship }) => {
    // Multi-proforma (cargo): PIs disponibles y desglose por producto.
    const S2_CARGO = !!(typeof window !== 'undefined' && window.PORTAL_CARGO);
    const s2Pis = (typeof window !== 'undefined' && window.PORTAL_PROFORMAS) || [];
    const s2PisForProduct = (pid) => s2Pis.filter(q =>
        !Array.isArray(q.product_ids) || !q.product_ids.length
        || q.product_ids.some(x => String(x) === String(pid)));
    const s2PiQty = (q, pid) => {
        const m = q.product_qtys || {};
        return parseFloat(m[String(pid)]) || 0;
    };
    const s2Containers = ((ship && ship.containers) || []).map(c => c.number).filter(Boolean);
    // Desglose "cuánto es de cada proforma" para un producto (solo cargo).
    ...
    // En pieza se pre-llena la cantidad total por si cambian a "suelto".
    ...
    const addGroup = (pid) => {
        ...
        // El bloque NUEVO se inserta AL PRINCIPIO: siempre queda arriba, a la
        // vista, sin obligar a hacer scroll hasta el fondo para llenarlo.
        return setDraft({ ...draft, blocks: [{
    ...
    const pkgUnit = (kind) => (kind === 'palet' || kind === 'pallet') ? 'palets' : (kind === 'tarima' ? 'tarimas' : 'cajas');
    ...
    const blockConfigured = (b, mode) => {
        // Cargo multi-proforma: sin PI el bloque no está declarado.
        if (window.PORTAL_CARGO && !b.pi_header_id)
            return false;
        if (mode === 'placa')
            return (+b.count || 0) > 0 && !!(b.name || '').trim();
        const pk = b.packaging || {};
    ...
    // PI (obligatoria en cargo) + Contenedor por bloque: lo que se elige aquí
    // se PRECARGA en todas las filas del bloque — sin propagar a mano.
    const s2AssignRow = (b, pid) => ((S2_CARGO || s2Containers.length > 0)
        ? React.createElement("div", { className: "block-fields-row" },
            (S2_CARGO && React.createElement(Field, { label: "Proforma (PI)", required: true },
                React.createElement(Select, { value: b.pi_header_id || '', onChange: (e) => updBlock(b.id, { pi_header_id: parseInt(e.target.value, 10) || false }) },
                    ...
                    // Solo el folio de la PI (sin el pedido de Odoo), igual
                    // que el selector por fila del packing list.
            ...
            (s2Containers.length > 0 && React.createElement(Field, { label: "Contenedor" },
                React.createElement(Select, { value: b.container || '', onChange: (e) => updBlock(b.id, { container: e.target.value || '' }) },
                    ...
                    s2Containers.map(c => React.createElement("option", { key: c, value: c }, c))))))
        : null);
    const renderProductCard = (p) => {
        ...
        // Total declarado por el proveedor (suelto: cantidad directa; empacado: nº de empaques).
        ...
        if (mode === 'placa') {
            ...
            const anyKind = groups.some(g => !!((g.packaging || {}).kind));
            if (allCfg) badge = { tone: 'done', icon: 'check', text: 'Declarado' };
            else if (anyCfg) badge = { tone: 'partial', icon: null, text: 'Declaración incompleta' };
            else badge = { tone: 'todo', icon: null, text: anyKind ? 'Falta cantidad enviada' : 'Falta indicar empaque' };
        }
        ...
        const showName = !caminoB || divided || mode === 'formato';
                ...
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 } },
                    ...
                    React.createElement("div", { style: { minWidth: 0 } },
                        ...
                        React.createElement("div", { className: "text-small", style: { fontWeight: 600, marginTop: 2 } },
                            ...
                            " · Declarado: ",
                            declaredLabel),
                        // CARGO multi-proforma: desglose de cuánto de este
                        // material corresponde a cada proforma.
                        (S2_CARGO && s2PiBreakdown(p).length > 0 && React.createElement("div", { style: { marginTop: 6, border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', maxWidth: 380 } },
                            React.createElement("div", { className: "text-small", style: { fontWeight: 700, padding: '4px 10px', background: 'var(--bg)', borderBottom: '1px solid var(--border)' } }, "Por proforma"),
                            s2PiBreakdown(p).map(x => React.createElement("div", { key: x.id, className: "text-small mono", style: { display: 'flex', justifyContent: 'space-between', gap: 12, padding: '3px 10px' } },
                                React.createElement("span", null, x.number),
                                React.createElement("span", { style: { fontWeight: 700 } }, x.qty + ((p.unit || '') ? ' ' + p.unit : '')))))))),
                ...
                return React.createElement("div", { key: b.id, className: "block-card", style: { borderColor: 'var(--border)' } },
                    (showPhoto && React.createElement("label", { className: `block-photo ${b.photo ? 'has-photo' : ''}`, style: { cursor: 'pointer', overflow: 'hidden' }, title: "Subir/Reemplazar foto" },
                        ...
                        blockPhotoSrc(b) ? React.createElement("img", { src: blockPhotoSrc(b), alt: "foto", style: { width: '100%', height: '100%', objectFit: 'cover', borderRadius: 8 } }) : React.createElement("div", { style: { textAlign: 'center' } },
                            ...
                            React.createElement("div", { style: { fontSize: 10, marginTop: 4, fontWeight: 600 } }, needsPhoto ? 'Foto del bloque' : 'Evidencia (opc.)')))),
                    React.createElement("div", { className: "block-fields" },
                        ...
                        (mode === 'placa' && React.createElement(React.Fragment, null,
                            React.createElement(Field, { label: `Nombre o número del bloque #${bi + 1}`, required: true },
                                React.createElement(Input, { mono: true, placeholder: 'Escribir aquí', value: b.name, onChange: (e) => updBlock(b.id, { name: e.target.value }) })),
                            React.createElement("div", { className: "block-fields-row" },
                        ...
                        (caminoB && React.createElement(React.Fragment, null,
                            (showName && React.createElement(Field, { label: mode === 'formato' ? ('Bloque/Tono' + (divided ? ` #${bi + 1}` : '')) : `Nombre del ${term}/lote #${bi + 1}` },
                                React.createElement(Input, { mono: true, placeholder: 'Escribir aquí', value: b.name, onChange: (e) => updBlock(b.id, { name: e.target.value }) }))),
                            React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
                                React.createElement("div", { style: { fontSize: 12.5, fontWeight: 600, color: 'var(--ink-3)' } }, "¿Cómo se envía este producto?"),
                                React.createElement("div", { className: "block-fields-row" },
                                    React.createElement(Field, { label: "Tipo de empaque", required: true },
                                    ...
                                    (hasKind && !loose && React.createElement(Field, { label: `Cantidad de ${pkgUnit(pk.kind)}`, required: true },
                                        React.createElement(Input, { mono: true, type: "number", min: 1, placeholder: "0", value: pk.qty || '', onChange: (e) => updPack(b.id, { qty: Math.max(0, +e.target.value || 0) }) }))))),
                            ...
                            (loose && React.createElement("div", { className: "block-fields-row", style: { marginTop: 2 } },
                                React.createElement(Field, { label: mode === 'formato' ? "Metros cuadrados enviados (total)" : "Cantidad total enviada", required: true },
                                    React.createElement(Input, { mono: true, type: "number", min: 0, value: b.count || '', placeholder: '0', onChange: (e) => updBlock(b.id, { count: Math.max(0, +e.target.value || 0) }) })))),
                            ...
                            (mode === 'formato' && hasKind && !loose && React.createElement("div", { className: "text-muted text-small", style: { padding: '2px 0' } },
                                React.createElement(Icon, { name: "info", size: 11 }), " Los m² se capturan después.")),
                            s2AssignRow(b, p.id),
                            React.createElement("div", { style: { display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'space-between', marginTop: 2 } },
                                React.createElement("span", { className: "text-muted text-small" }, mode === 'formato' ? 'Evidencia opcional del envío' : 'Sin evidencia requerida'),
...
const Step3Review = ({ proforma, draft, ship }) => {
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 } },
            ...
            statCard("Filas de la declaración", totalRows, true),
            statCard("Con detalles pendientes", pendingRows, false)),
        photosMissing > 0 && (React.createElement(Callout, { tone: "warn", icon: "alert", title: `${photosMissing} ${photosMissing === 1 ? 'bloque' : 'bloques'} sin foto` }, "Puedes continuar y subirlas despu\u00E9s, pero la declaraci\u00F3n del env\u00EDo no se considerar\u00E1 completa hasta que cada bloque tenga al menos una foto.")),
        React.createElement("div", { style: { marginTop: 18 } },
            React.createElement("div", { style: { fontSize: 12.5, fontWeight: 600, color: 'var(--ink-3)', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 10 } }, "Resumen de la declaración"),
            React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 12 } }, products.map(p => {
                ...
                return (React.createElement("div", { key: p.id, style: { border: '1px solid var(--border)', borderRadius: 12, padding: 16, background: 'var(--surface)' } },
                    ...
                    // Desglose por bloque: cantidad, PI y contenedor asignados
                    // (en cargo esto deja claro qu\u00E9 va de cada proforma).
                    React.createElement("div", { style: { marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 } },
                        draft.blocks.filter(b => String(b.product) === String(p.id)).map(b => {
                            const bMode = groupModeById(draft, proforma.products, b.product);
                            const bpk = b.packaging || {};
                            const qtyTxt = bMode === 'placa'
                                ? ((+b.count || 0) + ' placas')
                                : (bpk.kind === 'suelto'
                                    ? ((+b.count || 0) + (bMode === 'formato' ? ' m\u00B2' : ' unidades'))
                                    : ((+bpk.qty || 0) + ' ' + (bpk.kind || 'empaques')));
                            const bPi = ((typeof window !== 'undefined' && window.PORTAL_PROFORMAS) || []).find(x => x.id === b.pi_header_id);
                            return React.createElement("div", { key: b.id, className: "text-small mono", style: { display: 'flex', gap: 12, flexWrap: 'wrap' } },
                                React.createElement("span", { style: { fontWeight: 700 } }, b.name || (bMode === 'placa' ? 'Bloque' : 'Grupo')),
                                React.createElement("span", null, qtyTxt),
...
const Step4Sheet = ({ proforma, draft, setDraft, rows, setRows, ship, pendingImages, suppressNextRegen }) => {
    ...
    // Coincidencia EXACTA (dropdown estilo Excel): el valor del filtro es uno
    // de los valores existentes de la columna; '__empty__' filtra vacías.
    const cfMatch = (filterVal, rowVal) => {
        if (!filterVal) return true;
        const v = String(rowVal == null ? '' : rowVal).trim();
        if (filterVal === '__empty__') return !v;
        return v.toLowerCase() === String(filterVal).trim().toLowerCase();
    };
    const matchesColFilters = (r) => {
        const f = colFilters;
        return cfMatch(f.block, r.block)
            && cfMatch(f.atado, r.atado)
            && cfMatch(f.plate, r.plate)
            && cfMatch(f.thickness, r.thickness)
            && cfMatch(f.container, r.container)
            && (!f.pi || String(r.pi_header_id || '') === String(f.pi));
    };
    ...
    // Dropdown con los VALORES ÚNICOS presentes en la columna (como el
    // autofiltro de Excel), ordenados con criterio numérico natural.
    const colFilterInput = (key) => {
        const seen = new Set();
        const opts = [];
        let hasEmpty = false;
        allRows.forEach(r => {
        ...
        return React.createElement("select", {
            ...
            hasEmpty ? React.createElement("option", { value: "__empty__" }, "(vacías)") : null,
            opts.map(v => React.createElement("option", { key: v, value: v }, v)));
    };
    ...
    // Error = captura inconsistente (una dimensión sí y la otra no, en placas).
    const rowHasError = (r) => {
        if (PL_KIND(r) === 'placa') {
    ...
    const propagate = (sourceId, field, mode) => {
        ...
        if (mode === 'next') {
            ...
            for (let i = idx + 1; i < rows.length; i++) {
                // TOPE MANUAL: la propagación avanza solo hasta donde hay
                // No. Placa capturado. Un "0" corta SIEMPRE (separador de
    ...
    const PropCell = (props, children) => {
        ...
        const propable = canPropagate(rowId, field);
    ...
    // Celda de PI (factura de carga multi-pedido): el proveedor decide a qué
    // PI va cada fila; vacío = reparto automático. Con PropCell puede copiar
    ...
    const PASTE_NUMERIC = { w: 1, h: 1, quantity: 1 };
    ...
    const COL_DEFS = [
        ...
        { header: 'Grosor cm',  field: 'thickness', type: 'string' },
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' } },
            ...
            React.createElement("div", { style: { display: 'flex', gap: 5 } }, [
                ...
                { key: 'errors', label: 'Errores (', count: errorCount },
                    ...
                    if (kind === 'placa') {
                        table = React.createElement("table", { className: "sheet-table" },
                            React.createElement("thead", null, React.createElement("tr", null,
                                ...
                                (!NATIONAL && React.createElement("th", { style: { minWidth: 120 } }, "Bloque")),
                                (!NATIONAL && React.createElement("th", { style: { minWidth: 100 } }, "Atado")),
                                React.createElement("th", { style: { minWidth: 100 } }, "No. Placa"),
                                React.createElement("th", { style: { width: 100 } }, "Grosor cm"),
                            ...
                            React.createElement("tbody", null, gRows.filter(rowMatchesFilter).map((r) => {
                                ...
                                return React.createElement("tr", { key: r.id, className: `${isBlockStart ? 'block-start' : ''} ${activeRow === r.id ? 'is-active' : ''}`, onClick: () => setActiveRow(r.id) },
                                    ...
                                    PropCell({ rowId: r.id, field: "thickness" },
                                        React.createElement("input", { type: "text", value: r.thickness || '', style: { textTransform: 'uppercase' }, onPaste: onCellPaste(r, 'thickness', gRows), onChange: forceUpper((e) => updRow(r.id, { thickness: e.target.value })) })),
                        ...
                        table = React.createElement("table", { className: "sheet-table" },
                            React.createElement("thead", null, React.createElement("tr", null,
                                React.createElement("th", { style: { width: 30 } }, "#"),
                                React.createElement("th", { style: { minWidth: 110 } }, "Bloque/Tono"),
                            ...
                            React.createElement("tr", { className: "sheet-colfilters" },
                                colFilterClear(),
                                React.createElement("th", null, colFilterInput('block')),
                                React.createElement("th", null),
                                React.createElement("th", null, colFilterInput('plate')),
                                React.createElement("th", null),
                                React.createElement("th", null, colFilterInput('container')),
    ...
    return (React.createElement("div", null,
        ...
        React.createElement("div", { className: "card" },
            ...
            React.createElement("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 } },
                React.createElement(StatCard, { label: "Proforma", value: window.PORTAL_PI_LABEL(proforma.globals.proforma_number || '—'), mono: true }),
...
const ONBOARD_STEPS = [
    ...
    },
    {
        ...
        text: 'En el lado izquierdo verás el avance de cada sección con marcas visuales: verde = listo, azul = en progreso, gris = pendiente.',
    ...
    },
    {
        ...
        art: React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 6, fontSize: 11 } }, ['1. Productos', '2. Cómo se envía', '3. Revisión', '4. Completar detalles'].map((l, i) => (React.createElement("div", { key: l, style: { display: 'flex', alignItems: 'center', gap: 8 } },
...
function App() {
    ...
    const shipmentPayload = React.useCallback((snapshot, ship) => ({
        ...
        naviera_id: ship.naviera_id || false,
        forwarder_id: ship.forwarder_id || false,
        pol_id: ship.pol_id || false,
        pod_id: ship.pod_id || false,
        vessel_name: ship.vessel || '',
        port_origin: ship.port_origin || (snapshot.globals && snapshot.globals.port_origin) || '',
    ...
    const persistSnapshot = React.useCallback(async (snapshot) => {
        ...
        for (const ship of (snapshot.shipments || [])) {
            ...
            // Embarque ya recibido en tránsito: el backend rechaza cualquier
            // save_packing (candado). No se manda nada para no abortar el
            // autoguardado del resto de la proforma.
            if (ship.is_locked)
                continue;
    ...
    const completePortal = React.useCallback(async () => {
        try {
            ...
            await reloadPortal();
            // Los avisos internos (sobreasignación, PLs) NO se muestran al
            // proveedor: se asientan en el chatter de la OC para SOM.
            setNotice({ title: 'Confirmado', message: 'La proforma se marcó como completa. SOM GROUP recibió la notificación.', tone: 'ok', cta: 'Cerrar' });
        } catch (err) {
    ...
    return (React.createElement(LangCtx.Provider, { value: { lang, t: tFn } },
        React.createElement("div", { className: "app" },
            ...
            proforma.status === 'complete' && React.createElement("div", { className: "portal-complete-banner" },
                React.createElement(Icon, { name: "check", size: 14 }),
                React.createElement("strong", null, tr('Proforma completada')),
                React.createElement("span", null, tr('La proforma se marcó como completa. SOM GROUP recibió la notificación.'))),
